...
async def create_checkin(
    ...
    already_checked_in = await checkin_repo.check_already_checked_in_today(current_user_id, checkin.event_id)
    ...
    new_checkin = CheckIn(
        ...
        event_id=ObjectId(checkin.event_id),
        note=checkin.note,
        mood=checkin.mood
    )
    ...
    if streak.current_streak > 0:
        await user_repo.update_streak(current_user_id, streak.current_streak, streak.longest_streak)
...
async def get_checkin(
    checkin_id: str, repo: CheckInResponse = Depends(get_checkin_repository)
):
...
async def get_latest_checkin(
    user_id: str, repo: CheckInRepository = Depends(get_checkin_repository)
):
...
async def get_streak_status(
    ...
    try:
        ...
        # Check if already checked in today
        already_checked_in = await repo.check_already_checked_in_today(user_id, event_id)
        
        # Get today's date and the date of the last check-in
        today = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
        last_check_date = latest_checkin.check_date.replace(hour=0, minute=0, second=0, microsecond=0)
        ...
        if already_checked_in:
            ...
            has_freeze = await streak_freeze_repo.has_available_freeze(user_id, event_id)
